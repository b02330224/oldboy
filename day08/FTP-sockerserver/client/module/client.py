    ...
    def put(self,command):
        ...
        if os.path.isfile(file_name):
            ...
            send_msg='{cmd}|{fname}|{fsize}|{fmd5}'.format(cmd='put',
                                                           ...
                                                           fsize=fsize,
                                                           fmd5=fmd5)
            self.socket.sendall(bytes(send_msg,'utf8'))
            recive_server_data=str(self.socket.recv(2048),'utf-8')
            ...
            if if_can_send:
                #try:
                    with open(file_name,'rb') as f:
                        #根据返回获取文件已发送大小
                        sended_size=int(recive_server_data.split('|')[2])
                        while fsize-sended_size>2048:
                            f.seek(sended_size)
                            send_content=f.read(2048)
                            self.socket.send(send_content)
                            print(len(send_content))
                            sended_size+=len(send_content)
                            print('sended_size------------')
                            #打印上传进度条
                            common.print_proccess(fsize,sended_size)
                        else:
                            f.seek(sended_size)
                            send_content=f.read(fsize-sended_size)
                            l=len(send_content)
                            self.socket.send(send_content)
