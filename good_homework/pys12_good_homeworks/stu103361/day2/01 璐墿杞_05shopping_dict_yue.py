...
f1 = open("shopping_yue.txt","r")  #读出预算或者余额，余额变动后，再次写入到文件
# yue_dict = {}
for j in f1:  #遍历文件
...
while True: #外循环，输入商品编号
    ...
    if len(goods_no_input)==0: #如果商品编号为空
        ...
        if total < num: #如果需要支付的钱小于用户的预算
            ...
            print "\033[32;1m没有超出预算，还剩余%s元,可以继续购买\033[0m" % yue_list[-1]
...
#4将修改后的余额写入到新的文件
yue_list[-1] =  str(yue_list[-1])
yue_list[-2] =  str(yue_list[-2])
