...
while True: #外循环，输入商品编号
    ...
    if len(goods_no_input)==0: #如果商品编号为空
        ...
        if total < num: #如果需要支付的钱小于用户的预算
            ...
            left_money = num - total   #用户预算扣除购物车的花费后，计算用户的余额
            print "\033[32;1m没有超出预算，还剩余%s元,可以继续购买\033[0m" % left_money
            continue  #跳出循环的当前迭代，回到75行
        # print total
        if total >= num: #如果需要支付的钱大于等于用户的预算，钱不够了，计算用户余额和用户买这个商品还差多少钱
            cha_money = total - num #商品总价格-用户预算=用户差的钱
            left_money = shop_dict[goods_no_input]["price"]-cha_money
        #如果left_money为负数，取绝对值（用户差的钱+用户的余额=商品的价格）
            print "\033[31;1m钱不够了,还剩余%s元，想要买这件商品，还差%s元\033[0m" % (left_money,cha_money)
