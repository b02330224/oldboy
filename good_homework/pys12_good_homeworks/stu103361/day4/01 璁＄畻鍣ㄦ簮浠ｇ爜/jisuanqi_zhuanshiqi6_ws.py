...
def chengchu(expr_list):  #参数：列表（表达式作为列表的第一个参数） 乘除函数
    ...
    # print(mch)
    if not mch: #递归出口，最后没有乘除符号的时候，return跳出递归循环（函数终止），这里return不是返回None
        return   #
    ...
    # chengchu(expr_list)
    # return my_list[-1]
    chengchu(expr_list) #这个前面没有return，就是在递归计算的时候，不return，就是不依次return给上一层调用者
# 这个函数没有返回值，只是将去掉乘除符号的表达式，用于修改270行的expr_list值，这个270行的expr_list值修改后，加减函数还需要用到
...
def add_sub(expr_list):#参数：列表（表达式作为列表的第一个参数）
    ...
    # print(mch)
    if not mch: #递归出口，最后没有加减符号的时候，跳出递归循环，终止函数
        return
    ...
    add_sub(expr_list)#这个前面没有return，就是在递归计算的时候，不return，就是不依次return给上一层调用者
# 这个函数没有返回值，只是将去掉加减符号的表达式，用于修改280行的expr_list[0]值

def compute(expr): #参数：表达式  计算函数 括号里面的值传入
    # print(expr)
    expr_list = [expr,0] #关键点：乘除法计算完毕后，递归返回的是None，如何将乘除法计算后的值，传给加减法（解决办法：将表达式作为列表的第一个元素传入）
    # print("=",expr_list)  #= ['400/2', 0]
    chengchu(expr_list)  #调用乘除函数，参数是列表，列表的第一个元素是待计算的表达式
    # print("--",expr_list)  #  -- ['200.0', 0]  执行乘除函数后 expr_list已经修改了
